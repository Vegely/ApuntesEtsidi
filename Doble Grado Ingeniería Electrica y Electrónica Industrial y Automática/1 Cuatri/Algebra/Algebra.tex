...
\newpage
	...
	\begin{enumerate}
		\item Intercambio de líneas o permutaciones $F_{ij}, C_{ij} \rightarrow (i,j) \text{ Elementos que cambian}$
		\item Multiplicar por escalar $\lambda \ne 0 \quad F_i(\lambda) \quad C_i(\lambda)$ 
		\item Suma de dos líneas del mismo tipo y multiplicadas por escalar
		$$F_{ij}(\lambda), C_{ij}(\lambda)$$
		$$F_i + F_j(\lambda), C_i + C_j(\lambda)$$
	\end{enumerate}
	
	Si se realizan trasformaciones elementales por fila \(F\) o columna \(C\) equivale a multiplicar la matriz identidad por dichas transformaciones:
	$B \sim F \cdot A$ 
